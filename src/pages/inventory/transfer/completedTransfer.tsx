...
import CompleteTrfDet from "./completeTrfDet";
import {
...
const CompletedTransfer = () => {
  ...
  const { data: acceptedFrom, loading: acceptedFromLoading } = useQuery(GET_INVENTORY_TRANSFER, {
    variables: {
      status: "ACCEPTED",
      toShopId: null,
  ...
  const { data: acceptedTo, loading: acceptedToLoading } = useQuery(GET_INVENTORY_TRANSFER, {
    variables: {
      status: "ACCEPTED",
  ...
  const { data: declineFrom, loading: declineFromLoading } = useQuery(GET_INVENTORY_TRANSFER, {
    variables: {
      status: "DECLINED",
      toShopId: null,
      fromShopId: currentShop.shopId,
  ...
  const { data: declineTo, loading: declineToLoading } = useQuery(GET_INVENTORY_TRANSFER, {
    variables: {
      status: "DECLINED",
      toShopId: currentShop.shopId,
      fromShopId: null,
  ...
  const acceptedData: InventoryTransferAttr[][] = acceptedTo?.getInventoryTransfers.concat(
    acceptedFrom?.getInventoryTransfers
  );
  const declinedData: InventoryTransferAttr[][] = declineTo?.getInventoryTransfers.concat(
    declineFrom?.getInventoryTransfers
  );
  const data: InventoryTransferAttr[][] =
    useMemo(() => acceptedData?.concat(declinedData), [acceptedData, declinedData]) ?? [];
  console.log(data);
  ...
  const isFetching =
    acceptedFromLoading || acceptedToLoading || declineFromLoading || declineToLoading;

  return (
    <>
      <>
        <TrfHeader>
          <p className="black">Completed Transfers</p>
        ...
        <div style={{ height: "95%" }}>
                    ...
                    return (
                      ...
                      >
                        <Td
                          ...
                          style={{
                            color:
                              trf && trf[0]?.fromShopId === currentShop?.shopId
                                ? Colors.red
                                : Colors.green,
                          }}
                        >
          ...
          {reviewTrf && (
            ...
            >
              <CompleteTrfDet data={actionData} setShowModal={setReviewTrf} />
            </ModalSidebar>
          )}
        </div>
      </>
